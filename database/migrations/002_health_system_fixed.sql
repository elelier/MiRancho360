-- Migración CORREGIDA: Sistema de Salud y Tratamientos
-- User Story 1.2: Seguimiento de Salud y Tratamientos
-- VERSIÓN COMPATIBLE CON SCHEMA EXISTENTE (sin rancho_id)
...
-- Solo admins pueden insertar productos_salud
CREATE POLICY "Solo admins pueden insertar productos de salud"
    ON productos_salud
    FOR INSERT
    TO authenticated
    WITH CHECK (
...
-- Solo admins pueden actualizar productos_salud
CREATE POLICY "Solo admins pueden actualizar productos de salud"
    ON productos_salud
    FOR UPDATE
    TO authenticated
    USING (
...
-- Solo admins pueden eliminar productos_salud
CREATE POLICY "Solo admins pueden eliminar productos de salud"
    ON productos_salud
    FOR DELETE
    TO authenticated
    USING (
...
-- Agregar restricción única al campo nombre (necesario para ON CONFLICT)
ALTER TABLE productos_salud ADD CONSTRAINT productos_salud_nombre_unique UNIQUE (nombre);
